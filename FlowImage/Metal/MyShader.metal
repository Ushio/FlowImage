                       ...
                       texture2d<float, access::write> outputImage [[ texture(3) ]])
{
    ...
    // 外力
    float smoothstep_min = mix(2.0, 5.0, fuildConstant->pen);
    float smoothstep_max = mix(2.0, 25.0f, fuildConstant->pen);
    float dist = sqrt(SqDistPointSegment(fuildConstant->a, fuildConstant->b, gidf));
    nextU += fuildConstant->force * (1.0f - smoothstep(smoothstep_min, smoothstep_max, dist));
    ...
    // ダラダラと動かないように
    if(abs(blur.x) + abs(blur.y) < (1.0f / FLUID_SIZE))
    {
        blur.x = 0.0f;
        blur.y = 0.0f;
    }
